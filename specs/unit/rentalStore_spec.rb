...
describe RentalStore do
    before do
        ...
        allow(repo).to receive(:add_watched_movie)
    ...
    it "the rental store with two movies in its movie list, when verify if movie exists, and it doesn't, it has to be falsy" do
        @movie = Movie.new 0, "Back to the future", "Action", 134
        @rentalStore.add_movie(@movie)
